...
def configure_net(vocab_size, emb_dim, 
    ...
    model = SummarizerEncoder(**model_args)
...
def configure_training(opt, lr, clip_grad, lr_decay, batch_size):
    ...
    def criterion(logits, targets, sent_num):
        return sequence_loss(logits, targets, sent_num, ce, pad_idx=-1)
...
def train(args):
    ...
    # make model
    model, model_args = configure_net(len(word2id), args.emb_dim,
        args.conv_hidden, args.encoder_hidden, args.encoder_layer)
    
    # NOTE: the pretrained embedding having the same dimension
    #       as args.emb_dim should already be trained
    w2v_path='./IMGDM/word2vec/word2vec.128d.226k.bin'
...
def test(args, split):
    ...
    for i in range(min(5, len(ckpts))):
        ...
        extractor = Extractor(ext_dir, cur_ckpt, args, cuda=args.cuda)
...
if __name__ == '__main__':
    ...
    args.path = '{}{}_{}_{}'.format(args.path_root, 'Transformer', 'SL', 'W2V')
